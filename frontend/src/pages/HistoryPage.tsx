...
export function HistoryPage() {
  ...
  const { data, isLoading } = useQuery<AuditResponse>({
    ...
    queryFn: async () => {
      ...
      const logs: AuditEntry[] = json.logs ?? [];
      const filtered = logs.filter((e) => {
        if (search && !e.action.includes(search) && !e.resource.includes(search)) return false;
        if (actionFilter && !e.action.startsWith(actionFilter)) return false;
        return true;
      });
      const start = (page - 1) * LIMIT;
      return { entries: filtered.slice(start, start + LIMIT), total: filtered.length, page, limit: LIMIT };
    },
    staleTime: 30_000,
  });
